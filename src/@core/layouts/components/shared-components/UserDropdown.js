...
const UserDropdown = () => {
  ...
  if(profileError){
    return "Error in getting profile"
  }
  if(profileLoading){
  ...
  if(profileData.profiles.data.length>0){
    ...
    if(attrb&&attrb.photo.data&&attrb.photo.data.attributes){
      profileForDisplay.url=attrb.photo.data.attributes.formats.thumbnail.url
    }
        ...
        <Box sx={{ pt: 2, pb: 3, px: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Badge
              ...
              anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
            >
              <Avatar alt={"${profileForDisplay.firstname + ' ' +profileForDisplay.lastname}"} src={profileForDisplay.url} sx={{ width: '2.5rem', height: '2.5rem' }} />
            </Badge>
            <Box sx={{ display: 'flex', marginLeft: 3, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 600 }}>{`${profileForDisplay.firstname + ' ' +profileForDisplay.lastname}`}</Typography>
